...
function seedDB(connection, db) {
	if (db == "Works") {
		connection.query("SHOW TABLES LIKE 'Works'", function(error, rows, fields) {
			...
			// create Works table
			connection.query("CREATE TABLE Works (id INT PRIMARY KEY AUTO_INCREMENT NOT NULL, title VARCHAR(30), image VARCHAR(255), width FLOAT(16), height FLOAT(16), type VARCHAR(32), info VARCHAR(1024))", function(error) {
				// insert new data

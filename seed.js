...
function seedDB(connection, db) {
	if (db == "Works") {
		...
		connection.query("SHOW TABLES LIKE 'Posts'", function(error, rows, fields) {
			...
			// create Posts table
			connection.query("CREATE TABLE Posts (id INT PRIMARY KEY AUTO_INCREMENT NOT NULL, title VARCHAR(64), date VARCHAR(32), content VARCHAR(1024))");
			// insert new data
			connection.query("INSERT INTO Posts (title, date, content) VALUES ('First Post', '4/1/16', 'This is a dumby joke test post')");
			connection.query("INSERT INTO Posts (title, date, content) VALUES ('What is this title', '12/2/16', 'This is a dumby joke test post')");
			connection.query("INSERT INTO Posts (title, date, content) VALUES ('Third Post', '6/1/17', 'This is a dumby joke test post')");
		});
	}
}
